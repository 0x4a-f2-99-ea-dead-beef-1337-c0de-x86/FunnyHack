...
if ShyzosESP and ShyzosESP.Exit then
	ShyzosESP:Exit()
end
...
getgenv().ShyzosESP = {
	DeveloperSettings = {
		Path = "ShyzosESP/Configuration.cfg",
...
local Environment = getgenv().ShyzosESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("SHYZOS_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "SHYZOS_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("SHYZOS_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "SHYZOS_ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "SHYZOS_ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "SHYZOS_ESP.Exit: Missing parameter #1 \"self\" <table>.")
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "SHYZOS_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "SHYZOS_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "SHYZOS_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().ShyzosESP.DeveloperSettings = DeveloperSettings
	getgenv().ShyzosESP.Settings = Settings
	getgenv().ShyzosESP.Properties = Properties
	Environment = getgenv().ShyzosESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "SHYZOS_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "SHYZOS_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
