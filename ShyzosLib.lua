...
function utility.rgba(r, g, b, alpha)
	...
	if rgbasupported then
        ...
        warn("RGBA not supported in this executor.")
	end
...
local library = utility.table({theme = table.clone(themes.Shyzos), folder = "Shyzos V1", extension = "cfg", flags = {}, open = false, keybind = Enum.KeyCode.RightShift, mousestate = services.InputService.MouseIconEnabled, cursor = nil, holder = nil, connections = {}}, true)
...
function library:GetThemes()
	local themes = {"Default", "Midnight", "Shyzos"}
...
function library:Load(options)
	...
	local name = "Shyzos V1"
	local sizeX = options.sizex or 450
	local sizeY = options.sizey or 500
	local theme = options.theme and options.theme or "Shyzos"
